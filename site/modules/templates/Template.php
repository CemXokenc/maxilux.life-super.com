...
.sidenav>li>a {
 ...
 .sidenav>li {
	background: #fff;
	background: -moz-linear-gradient(top,rgba(245,245,245,1) 0,rgba(255,255,255,1) 75%);
	background: -webkit-gradient(linear,left top,left bottom,color-stop(0%,rgba(255,255,255,1)),color-stop(75%,rgba(221,221,221,1)));
	background: -webkit-linear-gradient(top,rgba(245,245,245,1) 0,rgba(255,255,255,1) 75%);
	background: -o-linear-gradient(top,rgba(245,245,245,1) 0,rgba(255,255,255,1) 75%);
	background: -ms-linear-gradient(top,rgba(245,245,245,1) 0,rgba(255,255,255,1) 75%);
	background: linear-gradient(to bottom,rgba(245,245,245,1) 0,rgba(255,255,255,1) 75%);
    ...
    <?php if( isset($site_variables["left_categories"]) && is_array($site_variables["left_categories"])){ ?>  
     ...
     <li <?php if( isset($site_variables["category"]["child"]) && is_array($site_variables["category"]["child"]) && count($site_variables["category"]["child"])>0){ ?>class="dropdown"<?php } ?>>
      <a  <?php if( isset($site_variables["category"]["child"]) && is_array($site_variables["category"]["child"]) && count($site_variables["category"]["child"])>0){ ?>href="javascript:;"<?php }else{ ?>href="<?php echo $site_variables["category"]["url"]; ?>"<?php } ?>>
       ...
       <?php if( $site_variables["category"]["articles_count"] > 0){ ?><span class="badge pull-right"><?php echo $site_variables["category"]["articles_count"]; ?></span><?php } ?>
      </a>       
...
<div id="content">
 <div id="content-header">
  <h1>Добавление категории склада</h1>
 </div> <!-- #content-header -->	
